...
use Smooler\Caches\Redis as CRedis;
...
abstract class WebSocketCore
{
	...
	final function handleShutdown($fd = 0, $responseId = 0) 
	{
    ...
    final function handleOnline($time) 
    {
	...
	final function handleOpen($request) 
	{
		try {
	...
	final function handleMessage($frame) 
	{
		try {
	...
	final function responseError($fd, $responseId, $res) 
	{
		$message = '';
